...
const Dashboard = () => {
  ...
  const getAllTasks = async () => {
    try {
      const res = await axios.get("https://test-ddm7.onrender.com/tasks", {
        headers: {
  ...
  const updateTask = async (id) => {
    try {
      const res = await axios.put(
        `https://test-ddm7.onrender.com/tasks/${id}`,
        {
          task,
          completed,
        },
  ...
  const deleteTask = async (id) => {
    try {
      const res = await axios.delete(`https://test-ddm7.onrender.com/tasks/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
